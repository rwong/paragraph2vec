...
from par2vec.models import par2vec
    ...
    def __iter__(self):
        with open(datapath('lee_background.cor')) as f:
            for i, line in enumerate(f):
                yield par2vec.TaggedDocument(utils.simple_preprocess(line), [self._tag(i)])
...
sentences = [par2vec.TaggedDocument(words, [i]) for i, words in enumerate(raw_sentences)]


def testfile():
    # temporary data will be stored to this file
    return os.path.join(tempfile.gettempdir(), 'par2vec_par2vec.tst')
...
class TestDoc2VecModel(unittest.TestCase):
    ...
    def test_empty_errors(self):
        ...
        self.assertRaises(RuntimeError, par2vec.Doc2Vec, [])

        # input not empty, but rather completely filtered out
        self.assertRaises(RuntimeError, par2vec.Doc2Vec, list_corpus, min_count=10000)
    ...
    def test_training(self):
        ...
        corpus = DocsLeeCorpus()
        model = par2vec.Doc2Vec(size=100, min_count=2, iter=20)
        model.build_vocab(corpus)
    ...
    def test_deterministic_hs(self):
        ...
        # hs
        model = par2vec.Doc2Vec(DocsLeeCorpus(), seed=42, workers=1)
        model2 = par2vec.Doc2Vec(DocsLeeCorpus(), seed=42, workers=1)
        self.models_equal(model, model2)
...
if __name__ == '__main__':
    # Previously level=logging.DEBUG
    logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.ERROR)
    logging.info("using optimization %s", par2vec.FAST_VERSION)
    unittest.main()
