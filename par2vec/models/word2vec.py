...
"""
Code from gensim/models with slight modifications.

Modifications
-------------
1.  Multilayer hs with backprop for cbow, nonlinear hidden units
...
try:
    from par2vec.models.word2vec_inner import train_batch_sg, train_batch_cbow
    from par2vec.models.word2vec_inner import score_sentence_sg, score_sentence_cbow
    from par2vec.models.word2vec_inner import FAST_VERSION, MAX_WORDS_IN_BATCH
except ImportError:
...
def train_cbow_pair(model, word, input_word_indices, l1, alpha, learn_vectors=True, learn_hidden=True):
    ...
    if model.hs:
        ...
        # Compute intermediate layers, linear
        for l, wgt in enumerate(model.wgts):
            layers.append(dot(layers[l], wgt.T))

        # Nonlinear hidden layers, beyond first hidden "input"
        #for l, wgt in enumerate(model.wgts):
        #    layers.append(1. / (1 + exp(-dot(layers[l], wgt.T))))

        last = layers[-1]
        # To be consistent with word2vec_inner code
        l2a = deepcopy(model.syn1[word.point]) # 2d matrix, codelen x layer1_size
        fa = 1. / (1. + exp(-dot(last, l2a.T)))  # propagate hidden -> output
        g = (1. - word.code - fa)  # vector of error gradients
        if learn_hidden:
            # Assuming nonlinear hidden units
            model.syn1[word.point] += outer(g * alpha, last)  # learn hidden -> output
            # Learn hidden(l) -> hidden(l+1)
            # Save error at hidden(l+1)
            for l in xrange(1, len(model.wgts) + 1):
                # Propagate errors
                g = dot(g, l2a)

                # Error gradient
                # Nonlinear component h_i = sum a_{ik}*g_k
                # so dh_i/da_{ik} = h_i(1-h_i)*g_k
                #g = g*layers[-l]*(1. - layers[-l])

                l2a = deepcopy(model.wgts[-l])
                model.wgts[-l] += outer(g * alpha, layers[-l - 1])
    ...
    if learn_vectors:
        ...
        for i in input_word_indices:
            model.syn0[i] += alpha * neu1e * model.syn0_lockf[i]
...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(
        ...
        if isinstance(size, int):
            ...
            self.layer_sizes = [size]
        else:
            self.vector_size = int(size[0])
            self.layer1_size = int(size[0])
            self.layer_sizes = size

        if self.layer_sizes[0] % 4 != 0:
    ...
    def train(self, sentences, total_words=None, word_count=0,
        ...
        def worker_loop():
            ...
            work = matutils.zeros_aligned(max(self.layer_sizes), dtype=REAL)  # per-thread private work memory
