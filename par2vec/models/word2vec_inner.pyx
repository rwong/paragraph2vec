...
scopy=<scopy_ptr>PyCObject_AsVoidPtr(fblas.scopy._cpointer)  # y = x
cdef saxpy_ptr saxpy
saxpy=<saxpy_ptr>PyCObject_AsVoidPtr(fblas.saxpy._cpointer)  # y += alpha * x
cdef sdot_ptr sdot
sdot=<sdot_ptr>PyCObject_AsVoidPtr(fblas.sdot._cpointer)  # float = dot(x, y)
cdef dsdot_ptr dsdot
dsdot=<dsdot_ptr>PyCObject_AsVoidPtr(fblas.sdot._cpointer)  # double = dot(x, y)
...
cdef void fast_sentence_cbow_hs(
    ...
    cdef long long a, b
    cdef long long row2
    cdef REAL_t f, g, ga, count, inv_count = 1.0
    cdef int m
    ...
    # Original code no locking on shared syn0 and syn1?
    # According to Mikolov, changes are small that it doesn't
    # make much of a difference.  See
    # https://groups.google.com/forum/#!searchin/word2vec-toolkit/
    #     thread/word2vec-toolkit/ms94b8b5QQQ/KeNTg99dCQAJ
    # Might be ok with original network configuration, but
    # this seems questionable for parameters in mulilayer configs
    ...
    # Assumes learn_hidden == True
    memset(work, 0, max_layer_size * cython.sizeof(REAL_t))
    for b in range(codelens[i]):
        row2 = word_point[b] * last_size
        f = our_dot(&last_size, last_layer, &ONE, &syn1[row2], &ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (1 - word_code[b] - f)
        ga = g * alpha
        # work same as neu1e in train_cbow_pair
        our_saxpy(&last_size, &g, &syn1[row2], &ONE, work, &ONE)
        our_saxpy(&last_size, &ga, last_layer, &ONE, &syn1[row2], &ONE)
