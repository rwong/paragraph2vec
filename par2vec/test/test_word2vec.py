...
from par2vec.models import word2vec
...
def testfile():
    ...
    return os.path.join(tempfile.gettempdir(), 'par2vec_word2vec.tst')
...
class TestWord2VecModel(unittest.TestCase):
    ...
    def testPersistenceWord2VecFormatWithVocab(self):
        ...
        model = word2vec.Word2Vec(sentences, min_count=1)
        model.init_sims()
        testvocab = os.path.join(tempfile.gettempdir(), 'par2vec_word2vec.vocab')
    ...
    def testTrainingCbowHsMulti(self):
        """Test word2vec training."""
        # build vocabulary, don't train yet
        model = word2vec.Word2Vec(size=[4,3,2], min_count=1, hs=1, negative=0)
        model.build_vocab(sentences)

        self.assertTrue(model.syn0.shape == (len(model.vocab), 4))
        self.assertTrue(model.syn1.shape == (len(model.vocab), 2))
        ...
        # build vocab and train in one step; must be the same as above
        model2 = word2vec.Word2Vec(sentences, size=[4,3,2], min_count=1, hs=1, negative=0)
        self.models_equal(model, model2)
