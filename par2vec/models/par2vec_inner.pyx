...
cdef unsigned long long fast_document_dmc_neg(
    ...
    cdef REAL_t *wgt_row,
    cdef REAL_t *last_layer = neu1
    cdef int l, r, last_size = layer1_size
            ...
            if _learn_words:
                for m in range(j, k):
                    if m == i:
                        ...
                        _ga = _word_locks[indexes[m]] * _alpha
    ...
    with nogil:
        for i in range(document_len):
            ...
            if _learn_doctags:
                for m in range(doctag_len):
                    _ga = _doctag_locks[_doctag_indexes[m]] * _alpha
                    our_saxpy(&vector_size, &_ga, &_work[m * vector_size],
                              &ONE, &_doctag_vectors[_doctag_indexes[m] * vector_size], &ONE)
            if _learn_words:
                for m in range(2 * window):
                    _ga = _word_locks[window_indexes[m]] * _alpha
                    our_saxpy(&vector_size, &_ga, &_work[(doctag_len + m) * vector_size],
